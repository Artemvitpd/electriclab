"""
Fast benchmark using optimized cache service.
"""
...
import commercial_service_fast as com
...
def warm_cached_reads(src_dir: str, cache_dir: str, files: List[str], ops: int, rng: random.Random) -> Tuple[float, float, int]:
    ...
    for _ in range(ops):
        ...
        with open(path, "rb") as f:
            data = f.read()
        if com.ENABLE_ENCRYPTION:
            _ = com.decrypt_data(data)
...
def main():
    ...
    with tempfile.TemporaryDirectory() as src, tempfile.TemporaryDirectory() as cache:
        ...
        print("\nFast cache warm-up (write to cache):")
        ...
        print(f"\nTime improvement (baseline vs fast cached): {improvement_time:.1f}%")
